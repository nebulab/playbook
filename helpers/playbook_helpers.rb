module PlaybookHelpers
  ...
  def_delegator :config, :playbook_section_topics_dir, :section_topics_dir
  ...
  def playbook_section_topics_files(section_page)
    topics_files_snapshot = topics_files
  ...
  def topics_files
    extensions[:file_watcher]
      ...
      .by_type(:source)
      .files
      .select { |file| file.relative_path.dirname.to_s == section_topics_dir }
  end
